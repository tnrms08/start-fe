...
function printItem(item) {
  ...
  <div style="margin:30px">
    <div><img src = "${item.img}" width="150""></div>
...
function printPage(page) {
  ...
  for (let i = 0; i < listSize; i++) {
    if (todayPhoto[startIndex + i] != null) {
      printItem(todayPhoto[startIndex + i]);
    }
  }
  $info.innerHTML = `${page}/${totalPage}`;
  //   pringItem(todayPhoto[startIndex]);
  //   pringItem(todayPhoto[startIndex + 1]);
  //   pringItem(todayPhoto[startIndex + 2]);
}
